				     ...
				     struct drgn_inlined_group **ret)
{
	...
	Dwarf_Die die;
	while(it->iter.entry && !dwarf_die_addr_die(it->dwarf, (void *)it->iter.entry->key, &die))
		it->iter = drgn_inlined_map_next(it->iter);
	...
	it->entry.die_addr = it->iter.entry->key;
	...
	for (struct uintptr_set_iterator uintptr_iter =
		...
		if (dwarf_entrypc(&die, &instance->entry_pc) != 0 || dwarf_highpc(&die, &instance->high_pc) != 0) {
			...
			if (offset == -1 || entry_pc == UINTPTR_MAX || high_pc == 0)
				continue;
	...
	it->entry.num_inlined_instances = num_valid_instances;
	*ret = &it->entry;
	return NULL;
}
...
static struct drgn_error *drgn_error_debug_info(struct drgn_module *module,
						const char *ptr,
						const char *message)
{
...
index_specification(struct drgn_debug_info *dbinfo, uintptr_t declaration,
		    struct drgn_module *module, uintptr_t addr)
{
...
skip:
		...
		if (depth == (tag == DW_TAG_enumerator ? 2 : 1) && name &&
			...
			if (insn & INSN_DIE_FLAG_DECLARATION)
				declaration = true;
			struct drgn_module *module = cu->module;
...
drgn_dwarf_info_update_index(struct drgn_dwarf_index_state *state)
{
	...
	#pragma omp parallel
	{
		...
		struct drgn_inlined_map *inlined_map =
			is_master_thread ? &dbinfo->dwarf.inlined_map :
			&(struct drgn_inlined_map)HASH_TABLE_INIT;
		...
		if (err) {
			...
			// Master thread has the global map as its local map,
			// so it shouldn't deinit on error.
			if (!is_master_thread)
...
struct drgn_dwarf_index_iterator {
	...
	uint32_t index;
	bool match_all;
	struct drgn_dwarf_index_die *current;
};
						...
						uint64_t *ret)
{
	...
	if (!*addr_base) {
		...
		err = drgn_module_cache_section(module, DRGN_SCN_DEBUG_ADDR);
		if (err)
			return err;
...
static struct drgn_error *drgn_dwarf_read_loclistx(struct drgn_module *module,
						   Dwarf_Die *cu_die,
						   uint8_t offset_size,
						   Dwarf_Word index,
						   Dwarf_Word *ret)
{
	...
	err = drgn_module_cache_section(module, DRGN_SCN_DEBUG_LOCLISTS);
	if (err)
		return err;
...
static struct drgn_error *drgn_dwarf5_location_list(struct drgn_module *module,
						    Dwarf_Word offset,
						    Dwarf_Die *cu_die,
						    uint8_t address_size,
						    uint64_t pc,
...
static struct drgn_error *drgn_dwarf4_location_list(struct drgn_module *module,
						    Dwarf_Word offset,
						    Dwarf_Die *cu_die,
						    uint8_t address_size,
						    uint64_t pc,
						    ...
						    size_t *expr_size_ret)
{
	...
	err = drgn_module_cache_section(module, DRGN_SCN_DEBUG_LOC);
	if (err)
		return err;
	struct drgn_debug_info_buffer buffer;
...
static struct drgn_error *
drgn_dwarf_location(struct drgn_module *module,
		    Dwarf_Attribute *attr,
...
#define PUSH_MASK(x) PUSH((x) & address_mask)
	...
	while (binary_buffer_has_next(&ctx->bb)) {
		...
		/* Register values. */
		case DW_OP_fbreg: {
			err = drgn_dwarf_frame_base(ctx->prog, ctx->locator, ctx->module,
		      ...
		      int *remaining_ops, uint64_t *ret)
{
	...
	struct drgn_dwarf_expression_context ctx;
	if ((err = drgn_dwarf_expression_context_init(&ctx, prog, module, locator,
		       ...
		       struct drgn_object *ret)
{
	...
	return drgn_object_from_dwarf_location(dbinfo->prog, module, &locator, die,
...
static struct drgn_error *
parse_member(struct drgn_debug_info *dbinfo, struct drgn_module *module,
...
static struct drgn_error *drgn_parse_dwarf_cie(struct drgn_module *module,
					       enum drgn_debug_info_scn scn,
					       size_t cie_pointer,
...
static struct drgn_error *
drgn_parse_dwarf_frames(struct drgn_module *module,
			enum drgn_debug_info_scn scn,
			...
			struct drgn_dwarf_fde_vector *fdes)
{
	...
	if (!module->scns[scn])
		return NULL;
	err = drgn_module_cache_section(module, scn);
	if (err)
		return err;
...
static void drgn_debug_info_cache_sh_addr(struct drgn_module *module,
					  enum drgn_debug_info_scn scn,
					  uint64_t *addr)
{
	if (module->scns[scn]) {
...
static struct drgn_error *
drgn_eval_dwarf_cfi(struct drgn_module *module, struct drgn_dwarf_fde *fde,
			       ...
			       void *buf, size_t size)
{
	...
	int remaining_ops = MAX_DWARF_EXPR_OPS;
	struct drgn_dwarf_expression_context ctx;
	drgn_dwarf_expression_context_init(&ctx, prog, regs->module, locator, NULL, NULL,
...
static struct drgn_error *drgn_location_descriptions_from_attribute(
	struct drgn_program *prog, struct drgn_module *module,
	...
	size_t *locations_size_ret)
{
	...
	do {
		...
		err = drgn_dwarf_location(module, &attr, regs, &location->expr,
					  &location->expr_size);
		if (err)
			break;
	...
	// When successful we intentionally do not deinit `locations`,
	// since we move its data into `locations_ret`.
	*locations_ret = locations.data;
	*locations_size_ret = locations.size;
	return NULL;
}
			 ...
			 struct drgn_object_locator *ret)
{
	...
	assert(prog && module && die && ret);
	dwfl_module_info(module->dwfl_module, NULL, &ret->module_start,
			 &ret->module_end, &ret->module_bias, NULL, NULL, NULL);
	struct drgn_error *err;
	err = drgn_type_from_dwarf_attr(prog->dbinfo, module, die,
					drgn_program_language(prog), false,
					false, NULL, &ret->qualified_type);
	if (err)
		return err;
	err = drgn_location_descriptions_from_attribute(prog, module, die,
							DW_AT_location, true,
							&ret->locations,
							&ret->locations_size);
	if (err)
	...
	if (function_die) {
		...
		err = drgn_location_descriptions_from_attribute(
			prog, module, function_die, DW_AT_frame_base, false,
...
struct drgn_type_iterator {
	...
	struct dwarf_index_iterator_vector namespace_iter_stack;
	struct drgn_qualified_type curr; // TODO check if used
					     ...
					     struct drgn_type_iterator **ret)
{
	...
	if (!iter)
		return &drgn_enomem;
	struct drgn_error *err;
	err = drgn_dwarf_index_iterator_init(&iter->type_iter, // TODO need to deinit?
...
struct drgn_error *drgn_type_iterator_next(struct drgn_type_iterator *iter,
					   struct drgn_qualified_type **ret)
{
	struct dwarf_index_iterator_vector *namespace_iter_stack = &iter->namespace_iter_stack;
	struct drgn_error *err;

	/*
	 * Attempt to get the next type in the current namespace
	 */
	struct drgn_dwarf_index_die *type_index_die;
	while (!(type_index_die = drgn_dwarf_index_iterator_next(&iter->type_iter))) {
		/*
		 * There are no more types in this namespace
		 *
		...
		struct drgn_dwarf_index_iterator *next_namespace_iter = dwarf_index_iterator_vector_append_entry(namespace_iter_stack);
		if (!next_namespace_iter)
			return &drgn_enomem;
		err = drgn_dwarf_index_iterator_init(next_namespace_iter, curr_namespace, NULL, 0,
		...
		Dwarf_Die ns_die;
		err = drgn_dwarf_index_get_die(next_namespace_index_die, &ns_die);
		if (err)
			return err;
		...
		// TODO deinit old type_iter
		struct drgn_namespace_dwarf_index *next_namespace = next_namespace_index_die->namespace;
	...
	/*
	 * We've found a type - extract it from the index die
	 */
	Dwarf_Die die;
...
//	 */
//	struct drgn_namespace_dwarf_index *curr_namespace;
//	if (namespace_dies->size > 0)
//		curr_namespace = namespace_dies->data[namespace_dies->size - 1].namespace;
//	else
//		curr_namespace = &iter->prog->dbinfo->dwarf.global;
//
...
//		if (!drgn_dwarf_index_die_vector_append(namespace_dies, ns_index_die))
//			return &drgn_enomem;
//
...
//	}
//	else {
//		// Push junk to balance stacks - it will be popped in the next step without being used
//		struct drgn_dwarf_index_die dummy_index_die = {0};
//		if (!drgn_dwarf_index_die_vector_append(namespace_dies, &dummy_index_die))
//			return &drgn_enomem;
//	}
//
//
...
//	 */
//	assert(namespace_iters->size == namespace_dies->size + 1);
//	if (namespace_dies->size > 0) {
//		// TODO deinit popped namespace_iter
//		dwarf_index_iterator_vector_pop(namespace_iters);
//		drgn_dwarf_index_die_vector_pop(namespace_dies);
//		string_builder_clear(&iter->namespace_name);
//		for (size_t i=0; i<namespace_dies->size; i++) {
//			Dwarf_Die die;
//			err = drgn_dwarf_index_get_die(&namespace_dies->data[i], &die);
//			if (err)
//				return err;
//
//			const char *ns_name;
...
//				return err;
//
//			string_builder_append(&iter->namespace_name, ns_name);
//		}
//		goto get_next_ns;
//	}
//
