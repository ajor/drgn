		...
		if (insn & INSN_DIE_FLAG_CHILDREN) {
			if (!enable_type_iterator && name && tag == DW_TAG_class_type) {
			  ...
			  if (!index_die(ns, cu, name,
			      DW_TAG_namespace, 0, cu->file, die_addr))
				return &drgn_enomem;
			}
			/*
						 ...
						 size_t *length_ret)
{
	...
	*bias_ret = module->debug_file_bias;
	...
	if (dwarf_getarangeinfo(dwarf_getarange_addr(aranges, pc), NULL, NULL,
                &offset) >= 0) {
        Dwarf_Die maybe_skeleton_die;
        if (!dwarf_offdie(dwarf, offset, &maybe_skeleton_die)) {
            return drgn_error_libdw();
        }
        uint8_t unit_type;
        if (dwarf_cu_info(maybe_skeleton_die.cu, NULL, &unit_type, NULL, NULL, NULL, NULL, NULL)) {
            return drgn_error_libdw();
        }

        if (unit_type == DW_UT_skeleton) {
            if (dwarf_cu_info(maybe_skeleton_die.cu, NULL, NULL, NULL, &maybe_skeleton_die, NULL, NULL, NULL)) {
                return drgn_error_libdw();
            }

            dwarf = dwarf_cu_getdwarf(maybe_skeleton_die.cu);
            offset = dwarf_dieoffset(&maybe_skeleton_die);
        }

        Dwarf_Die *cu_die = &maybe_skeleton_die;
			       ...
			       void *buf, size_t size)
{
	...
	int remaining_ops = MAX_DWARF_EXPR_OPS;
	struct drgn_dwarf_expression_context ctx;
	drgn_dwarf_expression_context_init(&ctx, prog, regs->module->debug_file, locator, NULL, NULL,
					   ...
					   struct drgn_qualified_type **ret)
{
	...
	if (err)
		return err;
	err = drgn_type_from_dwarf(iter->dbinfo, index_die->file, &die,
				   &iter->curr);
	if (err)
		return err;
	*ret = &iter->curr;
					   ...
					   struct drgn_qualified_type **ret)
{
	...
	if (err)
		return err;
	err = drgn_type_from_dwarf(iter->prog->dbinfo, index_die->file, &die,
				   &iter->curr);
	if (err)
		return err;
	*ret = &iter->curr;
